    ...
    C: RdConsumerExt + 'static,
{
    ...
    fn on_message(&self, msg_origin: &Self::DataType, origin_msg: &BorrowedMessage<'c>, _cr: &'c C::SelfType) {
        if let Some(msg) = UM::into(msg_origin) {
            let notify = TaskNotification::new(origin_msg.partition(), origin_msg.offset() as u64);
...
impl<T: Clone + Send> MsgDataPersistor<T, ()> {
    pub fn new(src: &DatabaseWriter<T>) -> Self {
        MsgDataPersistor {
...
impl<U> TopicConfig<U> {
    ...
    pub fn persist_to<T: Clone + Send>(self, db: &DatabaseWriter<T>) -> ChainedTopicBuilder<'_, T, Deco<U>, Self> {
        ChainedTopicBuilder {
            next_config: self,
            dbwriter: db,
            _phantom: PhantomData,
...
impl<'r> From<&'r super::UserMessage> for account::AccountDesc {
